...
{% hint style="warning" %}
**\[주의]** : 아래 안전 기능의 속도 제한값이 변경되는 경우, 변경된 제한값이 적용되기까지 최대 10 ms가 소요될 수 있습니다. 또한 Replan 기능의 경우, 현재 동작 상태에 따라 변경된 제한값을 만족할 때까지 감속 시간이 추가로 발생할 수 있으므로 이를 고려해야 합니다.
...
| 조인트 활성화 |   <p>각 관절의 활성화 여부</p><p>(활성화 / 비활성화)</p>  | 비활성화 |
| <p>속도</p><p>[mm/s]</p> |   <p>각 관절의 속도 제한값</p><p>(10 ~ 10000)</p>  | 1000.0 |
...
| **파라미터** |          **설명**                                                  |  **기본 설정값** |
| :------: | :----------------------------------------------------------------: | :---------: |
| 활성화 | <p>기능 활성화 여부</p><p>(무효 / 유효 / 안전 입출력 / 직접교시)</p> |  무효  |
| 정지 방법 |   <p>기능 위반시 정지 방법</p><p>(정지 0 / 정지 1 / 정지 2 / 무정지)</p>  | 정지 1 |
| 조인트 활성화 |   <p>각 관절의 활성화 여부</p><p>(활성화 / 비활성화)</p>  | 비활성화 |
...
| <p>지연 시간</p><p>[ms]</p> |   <p>Re plan으로 속도 변경시 지연시간 이후에 변경된 속도 제한값으로 감시함  </p><p>(0 ~ 2000)</p>  | 2000 |
| <p>속도 제한값</p><p>[mm/s]</p> |   <p>Re plan 이후 TCP 속도 제한값</p><p>(0 ~ 50000)</p>  | 50000 |
| <p>속도 비율</p><p>[%]</p> |   <p>Re plan 시 사용할 감속 비율</p><p>(0 ~ 100)</p>  | 100 |
| <p>입력 신호</p><p>[종류, 번호]</p> |   <p>Re plan을 위한 입력 신호</p><p>( [없음, -] / [기본 입력, 3] / [안전 입력, 0~7] / [PROFIsafe, 0~63] )</p>  | 0 |
...
<p align="center">
<img src="../../../_assets/safety_layout/space_orient.png"></img>
<em><p align="center">원점</p></em>
</p>

|  **파라미터** |                       **설명**                       |  **기본 설정값**  |
| :-------: | :------------------------------------------------: | :----------: |
| <p>Rx</p><p>[deg]</p> | <p>로봇좌표계 기준 X방향의 원점</p><p>(-180.0 ~ 180.0)</p> | 0 |
| <p>Ry</p><p>[deg]</p> | <p>로봇좌표계 기준 Y방향의 원점</p><p>(-180.0 ~ 180.0)</p> | 0 |
| <p>Rz</p><p>[deg]</p> | <p>로봇좌표계 기준 Z방향의 원점</p><p>(-180.0 ~ 180.0)</p> | 0 |
...
|  **파라미터** |                       **설명**                       |  **기본 설정값**  |
| :-------: | :------------------------------------------------: | :----------: |
| <p>Z 최소 / 최대</p><p>[mm]</p> | <p>로봇좌표계 기준 안전 영역의 높이</p><p>(-30000.0 ~ 30000.0)</p> |   0  |
...
| 형상 |   <p>툴 모델링 형태</p><p>(없음 / 구체 / 캡슐 / 둥근판)</p>  | 없음 |
...
| <p>X</p><p>[mm]</p> |  <p>X방향 모델 중심 위치</p><p>(-3000.0 ~ 3000.0)</p>  | 0 |
| <p>Y</p><p>[mm]</p> |  <p>Y방향 모델 중심 위치</p><p>(-3000.0 ~ 3000.0)</p>  | 0 |
| <p>Z</p><p>[mm]</p> |  <p>Z방향 모델 중심 위치</p><p>(-3000.0 ~ 3000.0)</p>  | 0 |
...
| 편차 각도 |   <p>툴 방향 제한값</p><p>(1.0 ~ 179.9)</p>  |  0.0 |
...
|:------------------------:|:----------------------------------------------------------------------------------------------------------------------------------:|:--------------:|:------:|
| 펄스 시험                 | 각 채널의 펄스 시험 사용 여부를 설정합니다.<br>펄스 시험이 활성화 된 채널의 경우 아래 그림과 같이 입력 쪽으로 테스트 펄스를 보내 수신된 펄스의 상태를 확인하여 케이블의 정상 상태를 판단합니다.                                                                                                    | 활성화 / 비활성화 | 비활성화 |
...
| 파라미터 <br>[단위]          | 설명                                                                                                                                       | 입력 범위       | 기본 값 |
|:------------------------:|:----------------------------------------------------------------------------------------------------------------------------------:|:--------------:|:------:|
| 펄스 시험                 | 펄스 시험 사용 여부를 설정합니다.<br>펄스 시험이 활성화 된 경우 아래 그림과 같이 테스트 펄스를 보내 수신된 펄스의 상태를 확인하여 출력의 정상 상태를 판단합니다.                                                                                                     | 활성화 / 비활성화 | 비활성화 |
...
4. 툴의 중량, 길이, 무게 중심과 이너셔를 설정한 후 `[OK]` 버튼을 터치하여 저장하십시오.
5. 변경된 툴 데이터를 안전 툴 데이터에 반영해주십시오. 안전 툴 데이터는 "[3.3.1.3 안전 툴 정보](../../3-safety-function/3-safety-function/1-general-condition/3-safe-tool-info.md)"를 참고하십시오.

{% hint style="info" %}
* 툴의 무게와 중심에 대한 정보가 없을 경우 부하추정 기능을 이용하여 값을 추정할 수 있습니다. 
* 툴 데이터 설정에 대한 자세한 내용은 "[Hi7 제어기 조작 설명서](https://hrbook-hrc.web.app/#/view/doc-hi6-operation/ko-tp630/README?cont_model=Hi7)"의 "[7.4.1 툴 데이터](https://hrbook-hrc.web.app/#/view/doc-hi6-operation/ko-tp630/7-system/4-robot-parameter/1-tool-data/README?cont_model=Hi7)"를 참고하십시오.
{% endhint %}
