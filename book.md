...
입력한 파라미터에 따라 로봇 모션을 자동으로 조정하는 기능입니다.
...
* 외부 장치에 안전 입력을 연결할 경우에는 안전 입출력 신호 및 정지 방법(정지 0, 정지 1, 정지 2)을 설정해야 합니다.
...
* 안전 정지 기능 설정에 대한 자세한 내용은 “[**3.3.1.9 안전 정지 기능**](../3-safety-function/1-robot-safety-condition/9-stop-function.md)”을 참조하십시오.
# 3.2.2 속도 및 위치 감시
...
* 로봇 감속에 대한 자세한 내용은 “[**3.3.1.6 속도 재조정 설정**](../3-safety-function/1-robot-safety-condition/6-re-plan.md)”을 참조하십시오.
* 안전 신호 설정에 대한 자세한 내용은 “[**3.3.3 안전 입출력**](../3-safety-function/3-safety-io/README.md)”를 참조하십시오.
# 3.2.3 동력 및 힘 제한
...
* **SOS**: 정지2 수행 후 축별로 비정상적인 움직임을 확인하여 로봇의 정지 상태를 감시
* **TCP 속도**: TCP 기준에서 로봇이 제한된 속도 이상으로 구동하지 못하게 제한
* **충돌 검지**: 로봇과 작업자의 충돌 발생 시의 압력을 제한
* **속도 재조정**: 외부 입력에 따라 로봇의 속도를 조절하여 협동 운전 모드 중 [3.2.2 속도 및 위치 감시](../../2-collaborative-operation-mode/2-speed-separation-monitoring.md) 수행
...
**\[시스템 > 8: 안전 시스템 > 2: 파라미터 설정 > 1: 로봇 제한 > 1: 조인트 영역]** 메뉴에서 파라미터 값을 설정할 수 있습니다.
...
| 활성화 | <p>기능 활성화 여부</p><p>(무효 / 유효 / 안전 입출력)</p> |   무효  |
| 조인트 활성화 |   <p>각 관절의 활성화 여부</p><p>(활성화 / 비활성화)</p>  | 비활성화 |
| <p>허용 범위</p><p>[deg]</p> |   <p>각 관절의 각도 제한값</p><p>(0.0 ~ 3.0)</p>  | 0.1 |

{% hint style="warning" %}
**\[주의]**: 정지 감시 파라미터 위반 시에는 재기동 전 로봇의 움직임이 정상인지 반드시 확인하십시오.  
{% endhint %}# 3.3.1.4 TCP 속도 제한 설정

로봇 좌표계 기준 TCP의 속도를 감시하는 기능입니다. 모니터링 위반 시에는 안전 정지(정지 0, 정지 1, 정지 2)가 즉시 활성화됩니다.

**\[시스템 > 8: 안전 시스템 > 2: 파라미터 설정 > 1: 로봇 제한 > 4: TCP 속도]** 메뉴에서 파라미터 값을 설정할 수 있습니다.
...
| :------: | :----------------------------------------------------------------: | :---------: |
| 활성화 | <p>기능 활성화 여부</p><p>(무효 / 유효 / 안전 입출력)</p> |  무효  |
| 안전 정지 |   <p>기능 위반시 정지 방법</p><p>(정지 0 / 정지 1 / 정지 2 / 무정지)</p>  | 정지 1 |
...
| <p>감속 비율</p><p>[%]</p> |   <p>속도 재조정 시 사용할 감속 비율</p><p>(0 ~ 100)</p>  | 0 |
| <p>지연 시간</p><p>[ms]</p> |   <p>속도 재조정으로 속도 변경시 지연시간 이후에 변경된 속도 제한값으로 감시함  </p><p>(0 ~ 1000)</p>  | 0 |
...
속도 재조정은 외부 안전 센서로부터 입력 받은 신호를 기반으로 로봇의 속도를 조절하는 기능입니다. 입력신호에 해당하는 감속비율로 로봇의 운전속도가 변경되고, 지연시간 이후에 해당하는 속도로 TCP 속도를 감시합니다. 
...
**\[시스템 > 8: 안전 시스템 > 2: 파라미터 설정 > 1: 로봇 제한 > 7: 파워]** 메뉴에서 파라미터 값을 설정할 수 있습니다.
...
| <p>최대 파워</p><p>[w]</p> |   <p>로봇의 파워 제한값</p><p>(80 ~ 1000)</p>  | 1000 |
...
**\[시스템 > 8: 안전 시스템 > 2: 파라미터 설정 > 1: 로봇 제한 > 8: 모멘텀]** 메뉴에서 파라미터 값을 설정할 수 있습니다.
...
| <p>최대 모멘텀</p><p>[kg·m/s]</p> |   <p>로봇의 모멘텀 제한값</p><p>(5 ~ 5000)</p>  | 1000 |
...
| <p>높이</p><p>[mm]</p> |  <p>플레이트의 높이</p><p>(0.0 ~ 5000.0)</p>  | 0 |
...
# 3.3.3.2 확장 안전 입출력 신호

부가 안전 입출력 신호의 파라미터를 설정합니다. 입력 신호 8개, 출력 신호 8개로 구성되며 모두 이중신호로 동작 됩니다.
**\[시스템 > 8: 안전 시스템 > 2: 파라미터 설정 > 3: 안전 입출력 > 3: 확장 입출력]** 메뉴에서 파라미터 값을 설정할 수 있습니다. 

## 1. 부가 안전 입출력 신호

<p align="center">
<img src="../../../_assets/safety_io/add_io.png"></img>
<em><p align="center">확장 입출력 설정 화면</p></em>
</p>

| 파라미터 [단위]             | 설명                                                                                                                                       | 입력 범위       | 기본 값 |
|:---------------------------:|:----------------------------------------------------------------------------------------------------------------------------------------:|:--------------:|:------:|
| 활성화                       | 확장 안전 입출력 신호의 활성화 / 비활성화 여부를 설정합니다.                                                                                       | 활성화 / 비활성화 | 비활성화 |
| 필터 시간 <br>[msec]          | 각 입력 채널별로 **필터 시간** 동안 일정한 신호가 입력되어야 유효한 신호로 처리됩니다.<br>5로 나누어 떨어지는 값만 입력 가능합니다.                           | 0 ~ 500        | 100    |
| 불일치 허용 시간 <br>[msec]     | 확장 안전 입출력은 2개의 이중 신호가 같은 값일 때 유효한 신호로 처리됩니다.<br>이 2개의 신호가 **불일치 허용 시간**보다 큰 시간 동안 다르면 알람이 발생합니다.<br>5로 나누어 떨어지는 값만 입력 가능합니다. | 0 ~ 5000       | 1000   |
| 입력 오류 유지 시간 <br>[msec] | 각 채널은 에러가 발생한 후 해소되더라도, 설정된 시간이 지난 후에야 Fail-Safe 상태에서 현재 입력 상태로 전환됩니다.<br>5로 나누어 떨어지는 값만 입력 가능합니다.             | 0 ~ 65530      | 1000   |
| 출력 오류 유지 시간 <br>[msec] | 각 채널은 에러가 발생한 후 해소되더라도, 설정된 시간 동안 **Open (Fail-safe)** 상태를 유지합니다.<br>이후 정상 출력으로 전환됩니다.<br>5로 나누어 떨어지는 값만 입력 가능합니다.   | 0 ~ 65530      | 1000   |
...
<p align="center">
<img src="../_assets/5/PROFINET_monitoring.png"></img>
...
<p align="center">
<img src="../_assets/5/PROFISAFE_monitoring.png"></img>
