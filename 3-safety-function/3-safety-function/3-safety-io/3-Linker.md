...
| SGG| 안전 가드 일반 입력| 
| SGA | 안전 가드 자동 입력|
| Protective stop | 보호 정지 입력|
| Normal stop | 정상 정지 입력|
...
| Monitored standstill #1 ~ #8 | 안전 정지 활성화<br>(sos_0~sos_7) |
| Joint speed set #1 ~ #8 | 조인트 속도 감시 활성화<br>(speed_0~speed_7) |
| TCP speed set #1 ~ #16 | TCP 속도 활성화<br>(speed_0~speed_15) |
| Joint angle #1 ~ #8 | 조인트 영역 감시 활성화<br>(space_0~space7) |
| TCP position(space) #1 ~ #16 | TCP 영역 감시 활성화<br>(space_0~space15) |
| TCP orientation #1 ~ #8 | 툴 방향 감시 활성화<br>(orient_0~orient7) |
| Self collision | 자가 충돌 감시 활성화|
| Power #1 ~ #16 | 파워 감시 활성화<br>(power_0~power_15) |
| Momentum #1 ~ #16 | 모멘텀 감시 활성화<br>(mmt_0~mmt_15) |
| Collision detection #1 ~ #16 | 충돌 검지 활성화<br>(coldet_0~coldet_15) |
| Speed & separation #1 ~ #84 | RePlan 활성화 |
